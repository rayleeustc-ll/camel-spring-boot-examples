...
public class CamelRouter extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:hello").setHeader("name").constant("lelewang").setHeader("action").constant("test")
                .transform().constant("Hello World").log("header name is ${header.name}").log("header action is ${header.action}")
                .log("my properties is {{hello}} {{myproperty}}");
        from("direct:bye")
        ...
        rest("/users").description("User REST service")
            ...
            .get("/{id}").description("Find user by ID")
                ...
                .to("activemq:PEOSONEQUEUE?exchangePattern=InOnly")
        ...
        from("direct:update-user")
            .to("bean:userService?method=updateUser").setBody().constant("update successfully").log("rp ${body}")
            .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(204));

        //test file
        from("file://input_box?noop=true").to("file://output2_box");
    }
